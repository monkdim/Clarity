...
The roadmap is complete. All seventy-five phases shipped:
...
- **Phase 75** (this commit) — perf profiling + crash recovery + branding + website + release tooling.

What `clarity os build && clarity os run` does today on a Mac with Zig + QEMU installed: builds the kernel, builds the freestanding runtime (with QuickJS vendored via `runtime/freestanding/third_party/fetch_quickjs.sh`), packs the ISO via the Clarity-side ISO9660 packer, launches QEMU with HVF + virtio-gpu + virtio-keyboard + OVMF auto-discovered. The kernel boots, hands off to `/bin/clarity-init`, init mounts tmpfs/devfs/procfs, spawns the desktop session, the Aurora boot splash fades out, and the desktop appears with the dock pre-pinned to terminal / files / editor / calc / viewer / monitor.
