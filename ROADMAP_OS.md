...
| # | Task | Status | Description |
|---|------|--------|-------------|
| 1 | **Performance profiling** | Done | `stdlib/perf_profiler.clarity` — `BootProfile` (phase timeline + slowest_phase + total_ms vs 5 s target), `FrameProfile` (sliding 240-frame window + average + p99 + dropped count, 16.67 ms target), `MemoryProfile` (labelled snapshots, 256 MB idle target), `AppLaunchProfile` (per-app click→first-paint, 250 ms target). `ReleaseGate(boot, frame, memory, app_launch)` aggregates the four; `passes()` is the green-light call the release pipeline checks before tagging. |
| 2 | **Crash recovery** | Done | `stdlib/crash_recovery.clarity` — `Journal` (severity-filtered + capacity-bounded ring buffer with subscribers + optional file persistence; debug/info/warn/error/fatal helpers + tail/filter/clear), `Watchdog` (per-service exponential backoff: 250ms → 500ms → 1s → 2s …; circuit breaker trips after 5 crashes inside 60 s and stays tripped until reset; pluggable factory), `CrashDialog` (state machine: initial → details / dismissed / reporting → reported; pluggable bug reporter so apps can hand off to GitHub issues / Sentry / etc.). |
| 3 | **Theming & branding** | Done | `stdlib/branding.clarity` — `BRAND_NAME`/`BRAND_TAGLINE`/`BRAND_DOMAIN` constants, design tokens (8-step typography scale, 6-step spacing scale, radius scale + token lookup helpers), procedurally-painted ClarityOS logo (open ring + downward arrow, gradient-coloured around the ring), gradient wordmark (each glyph samples a different point on the Aurora gradient), 11 procedural app icons for terminal / files / editor / calc / viewer / monitor / browser / mail / chat / store / settings — each a recipe + small glyph-paint routine, all resolution-independent so the dock + launcher draw them at any size from one source. |
| 4 | **Website & downloads** | Done | `stdlib/website_gen.clarity` — `Website` builder emits 6 pages (index / download / docs index / community / og preview / shared CSS) into a target directory. Aurora-themed CSS using the same teal → violet → magenta gradient as the boot splash (CSS custom properties + `linear-gradient` background-clip on the wordmark). Download page renders ISO size + SHA256 + macOS dd + QEMU instructions. OG card for share-link previews. `build()` returns a pure path → content map for tests; `build_to_dir()` writes them. |
| 5 | **ClarityOS 1.0 release** | Done | `stdlib/release.clarity` — `Release` orchestrates the full pipeline: validate (clean tree, on `main`, tag doesn't exist) → run release gate → generate changelog from `git log` since the previous tag → build artefacts via injected builder → SHA256 each → compose release notes (header + downloads + changelog) → create + push tag → publish via injected publisher (gh release create or equivalent). Dry-run mode skips the publish step. Pluggable git / builder / publisher backends so the test suite drives the same pipeline against fakes. |

**Tests:** `stdlib/test_polish.clarity` — 106 assertions: boot profile (timeline + total + passes_target + slowest_phase = whichever phase actually took longest); frame profile (60 frames at 15 ms passes the gate, fps > 60, dropped frames counted when 40 ms frames slip in); memory profile (200 MB passes, 300 MB fails); app launch (multiple apps under 250 ms passes, 400 ms fails, slowest detection); release gate aggregates all four (passes only when every component passes; fails when boot regresses); journal (3 entries → tail returns last 2; severity filter drops below-threshold logs; subscriber notified on log; capacity rolls oldest out when exceeded); watchdog (1st crash → 100 ms backoff; tick before backoff = no restart; tick after = factory invoked + state→running; 3 crashes inside window trip the breaker; reset returns to registered); crash dialog (initial → details ↔ details; dismiss action; restart action carries source; report_bug with reporter returns report_id + state→reported, without reporter returns null); branding tokens (typography + spacing + radius scale lookups + fallbacks; all 11 app icons in the recipe table with the right accent colours); website (6 pages; index has brand+tagline+CTA; download has SHA256+size+QEMU instructions; CSS has aurora vars; og card has og:title+twitter:card; build_to_dir writes files); release pipeline (clean run with fake git+builder+publisher creates+pushes tag, invokes builder once, invokes publisher once; changelog includes Phase 75; release notes include ISO name + SHA256; dirty tree blocks; failing gate blocks; dry-run skips publish step).

---

## ClarityOS 1.0 — Status

The roadmap is complete. All seventy-six phases shipped:

- **Phases 56–63** (FFI through core apps) — graphics, input, window manager, UI toolkit, system services, desktop shell, default apps.
- **Phases 64–69** (app framework through dev tools) — manifest + sandbox + store, micro-kernel + runtime + bootable ISO, web/networking apps, IDE/UI builder/docs/playground.
- **Phases 70–74** (kernel completion through bootable on macOS) — real ELF loader + ring switch + SYSCALL fast path + page faults + timer + fork/exec/wait/kill + multiboot framebuffer; freestanding QuickJS runtime + libc shim + TTY + devfs + per-process fd table; userspace bootstrap + procfs + input pipeline; compositor on bare metal + display server + desktop session; pure-Clarity ISO9660 packer + macOS QEMU launcher + Aurora theme + wallpapers + animations + boot splash.
- **Phase 75** — perf profiling + crash recovery + branding + website + release tooling.
- **Phase 76** (this commit) — spring design refresh: Meadow / Bloom / Watercolor light themes (Meadow is the new default), Aurora demoted to "Midnight" for users who prefer dark, asymmetric meadow wallpaper + asymmetric boot splash, flat two-tone Drop+Leaf logo (no skeuomorphic highlight), abstract app-icon marks, theme registry + theme picker that wires into the existing settings panel.

### Phase 76 — Spring design refresh

| Step | Module | What |
|---|---|---|
| 1 | **Meadow / Bloom / Watercolor palettes** | `stdlib/theme_meadow.clarity` — three light spring palettes built on shared design tokens (radius, spacing, typography, motion). Meadow is sage + daffodil + blossom + sky on a cream canvas. Bloom is mint + apricot + coral. Watercolor is dreamy pastels on parchment. `brand_gradient(theme, t)` samples sky → primary → secondary; `warm_gradient` samples secondary → tertiary → highlight. `_lerp_color` reconstructs ARGB by integer arithmetic so the high byte of `0xFF......` constants doesn't sign-extend. |
| 2 | **Spring wallpapers** | `stdlib/wallpapers_spring.clarity` — `paint_meadow_morning` paints a 48-band vertical gradient (canvas → muted primary → muted sky) with a soft horizon stripe at 55% of the canvas. Asymmetric anchor without per-pixel work; uses `fill_rect` (one runtime helper per band) so it stays cheap in interpreted mode. Bloom uses the same recipe with the bloom theme; Watercolor paints three soft horizontal washes at 20/50/78% with alpha-blended tint. |
| 3 | **Modern branding kit** | `stdlib/branding_modern.clarity` — flat two-tone `paint_logo` (drop in primary, leaf carved into the bulb in a lighter tone, no white highlight, no veins). `paint_wordmark` is single-colour deep ink with letter-spacing. `paint_icon` paints a rounded surface + top accent stripe + a single bold abstract mark per app (wave / stack / stripes / grid / frame / pulse / ring / envelope / bubble / diamond / cog) instead of literal glyphs. Mirrors the typography / spacing / radius scales from theme tokens. |
| 4 | **Modern boot splash** | `stdlib/boot_splash_modern.clarity` — same external API as `boot_splash.clarity` (`set_progress`, `fade_out`, `tick`, `done`) so `init.clarity` doesn't change. Asymmetric layout: logo + wordmark left-aligned in the upper third (not centred), tagline below in muted ink, full-width progress bar at the very bottom with the brand gradient sampled across the fill. Spinner is three short arcs sampling the gradient, not a single-colour ring. |
| 5 | **Theme registry** | `stdlib/theme_registry.clarity` — registers the four built-ins (Meadow / Bloom / Watercolor / Midnight where Midnight = Aurora re-labelled with `id="midnight"`, `scheme="dark"`). `set_active(id)` persists to `~/.clarity-os/theme`; `default_registry()` factory loads the persisted choice on session start. Apps can `register(id, theme)` their own palettes. |
| 6 | **Theme picker** | `stdlib/theme_picker.clarity` — pure-state engine the existing settings panel mounts as the "Appearance" pane. `options()` returns `{id, name, scheme, active, swatches}` quintets the UI paints as a grid; `light_options()` / `dark_options()` filter by scheme; `select(id)` calls `registry.set_active` + fires the `on_change` callback; `preview(id)` / `clear_preview()` track hover/keyboard preview state separate from the active selection; `snapshot()` exposes the picker state for the renderer. |

**Tests:** `stdlib/test_spring_refresh.clarity` — 112 assertions: theme palette structure (id, name, scheme, primary/secondary/tertiary/highlight/background hex codes, design tokens present, window radius = 14, md spacing = 12); `brand_gradient` endpoints (gradient(0) = highlight, gradient(0.5) = primary, gradient(1.0) = secondary; clamping for negative + `>1` t); `warm_gradient` endpoints; `_lerp_color` preserves the ARGB high byte (no sign-extension regression); registry built-ins (4 themes; meadow as default; midnight is dark scheme; get/set_active/active_id; register custom theme); persistence (set_active writes the file; new registry on the same dir loads it; missing file = false); theme picker options (4 swatches per option; exactly one active; light_options = 3, dark_options = 1; select fires on_change; preview/clear_preview/current_preview track hover state; snapshot shape); branding tokens (typography/spacing/radius lookups + fallbacks; all 11 app icons present in the recipe table with abstract marks); paint smoke tests for `paint_logo` + `paint_icon` + `paint_wordmark` + a tiny boot splash buffer (verify the painters wire without exhausting interpreter cycles); progress clamping (`>1` → 1, `<0` → 0, status text updates); fade-out animation (not done immediately, done after duration); spring preset catalogue (4 presets, all ids present, unknown returns false).

What `clarity os build && clarity os run` does today on a Mac with Zig + QEMU installed: builds the kernel, builds the freestanding runtime (with QuickJS vendored via `runtime/freestanding/third_party/fetch_quickjs.sh`), packs the ISO via the Clarity-side ISO9660 packer, launches QEMU with HVF + virtio-gpu + virtio-keyboard + OVMF auto-discovered. The kernel boots, hands off to `/bin/clarity-init`, init mounts tmpfs/devfs/procfs, spawns the desktop session, the Meadow boot splash fades out, and the desktop appears on the cream-and-sage canvas with the dock pre-pinned to terminal / files / editor / calc / viewer / monitor. The first run picks Meadow; users can switch to Bloom, Watercolor, or Midnight from Settings → Appearance, and the choice persists to `~/.clarity-os/theme`.
