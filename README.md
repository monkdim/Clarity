...
- **Self-hosted.** The lexer, parser, interpreter, bytecode VM, type checker, linter, formatter, debugger, profiler, doc generator, package manager, language server, transpiler and shell are all written in Clarity. **64 test files, about 3,500 assertions, all in Clarity**, run in CI on macOS ARM64, Linux x64 and Linux ARM64. One honest caveat: rebuilding the toolchain with the self-hosted compiler alone still depends on the Python bootstrap for twelve modules, because the two bundlers keep separate hard-coded module lists. That is tracked in [GAPS.md](GAPS.md).
...
- **Ships as a single binary.** Clarity transpiles to JavaScript and Bun compiles the bundle to a native executable for macOS and Linux, x64 and ARM64. The toolchain needs nothing installed on the target machine. Programs that import standard-library modules still need the `stdlib/` sources on disk beside them; embedding the library in the binary is on the list.
...
| | Clarity | KyanOS |
|---|---|---|
| **Status** | v1.0.0. Builds and runs from source. Release binaries are not yet attached to a release, so `install.sh` and Homebrew do not work until they are. | Experimental. Two kernels boot and run programs under QEMU on every commit. The desktop runs hosted and in the browser; boot-to-desktop on the kernels is not yet built. |
...
| **Tests** | 64 test files, about 3,500 assertions, `clarity test stdlib/` on three targets | 16 boot markers on x86_64 (three boots), 32 on aarch64 (three machine sizes), plus screenshot, keyboard and serial checks |
...
Until a release with binaries is published, build from source. You need [Bun](https://bun.sh) and Python 3 for the bootstrap.
...
Once a release carries binaries, `curl -fsSL https://raw.githubusercontent.com/monkdim/Clarity/main/install.sh | bash` is the one-line install.
