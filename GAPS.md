...
- **Native compilation (Clarity → C → binary).** *In progress.* `clarity cc` compiles the scalar core (stage 1); lists, maps, `for`-loops over lists/maps/strings/ranges, indexing + index-assignment, and the common builtins (`len`/`push`/`str`/`int`/`float`/`range`/`keys`/`has`/`contains`) (stage 2); **and now classes** — instances (class + field map), methods with runtime dispatch, constructors, `this` field access, `to_string`/default display, and single inheritance (stage 3). Collections and instances render exactly like the interpreter, and every codegen test compiles the C with a real compiler and diffs the native output against the tree-walking interpreter (34 cases). **Next:** closures (`fn` expressions that capture their environment — the one gap that blocks compiling `map`/`filter` with inline lambdas), then a real memory strategy (arena/GC instead of the stage-1/2/3 leak). Endgame: `clarity build` emits a native binary with **no Bun**. (This supersedes the stalled Zig `runtime/native_vm/` bet — the C path is fully buildable and testable in CI, and produces compiled code rather than a bytecode interpreter in a binary.)
