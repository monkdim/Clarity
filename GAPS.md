...
- **Native compilation (Clarity → C → binary).** *In progress.* `clarity cc` works for the scalar core (stage 1). **Next:** stage 2 — lists, maps, `for`-loops, and the common builtins (`len`/`push`/`str`/`range`/`keys`…); stage 3 — classes, closures, and a real memory strategy (arena/GC instead of the stage-1 leak). Endgame: `clarity build` emits a native binary with **no Bun**. (This supersedes the stalled Zig `runtime/native_vm/` bet — the C path is fully buildable and testable in CI, and produces compiled code rather than a bytecode interpreter in a binary.)
